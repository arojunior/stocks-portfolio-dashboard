...
class LiveTelegramMonitor:
    ...
    async def get_available_channels(self) -> List[Dict]:
        ...
        try:
            ...
            st.error(f"Error getting channels: {e}")
            return []
...
def display_live_telegram_dashboard():
                    ...
                    if success:
                        ...
                        if channels:
                            ...
                            for i, channel in enumerate(channels[:10]):  # Show first 10
                                st.write(f"{i+1}. {channel['title']} ({channel['participants_count']:,} members)")
                            
                            if len(channels) > 10:
                                st.write(f"... and {len(channels) - 10} more channels")
    ...
    **Step 2: Select Channels**
    - Choose channels you want to monitor
    - Set your monitoring preferences
    - Click "Start Live Monitoring"
    
    **Step 3: View Results**
    - See real-time stock mentions
