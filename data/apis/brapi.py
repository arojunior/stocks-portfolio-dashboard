...
def fetch_fii_dividend_analysis(ticker: str) -> Optional[Dict]:
    ...
    try:
        ...
        if api_key:
            ...
            params = {
                "token": api_key,
                "range": "3mo",  # Get 3 months of data (free plan limit)
                "interval": "1d"
            }
        else:
            url = f"https://brapi.dev/api/quote/{clean_ticker}"
        ...
        if "results" in data and data["results"]:
            ...
            total_dividends_3mo = 0
            dividend_count_3mo = len(dividends)

            for dividend in dividends:
                if dividend.get("value"):
                    value = float(dividend["value"])
                    date = dividend.get("date", "")
                    total_dividends_3mo += value
            ...
            # Calculate metrics
            # Calculate monthly average from 3 months of data
            avg_monthly_dividend = total_dividends_3mo / 3 if dividend_count_3mo > 0 else 0
            # Annualize the 3-month data
            annual_dividend_yield = (total_dividends_3mo * 4) / current_price * 100 if current_price > 0 else 0
            monthly_dividend_yield = annual_dividend_yield / 12 if annual_dividend_yield > 0 else 0

            # Calculate projected annual income (per share) - annualize 3-month data
            projected_annual_income = (total_dividends_3mo * 4) if dividend_count_3mo > 0 else 0
            ...
            return {
                ...
                "current_price": current_price,
                "dividend_history": monthly_dividends,
                "total_dividends_3mo": total_dividends_3mo,
                "dividend_count_3mo": dividend_count_3mo,
